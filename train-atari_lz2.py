...
class Model(ModelDesc):
    ...
    def _build_graph(self, inputs):
        state, action, oldpi, vpred_old, atarg, weight = inputs
...
class MySimulatorMaster(SimulatorMaster, Callback):
    ...
    def _on_datapoint(self, ident):
        client = self.clients[ident]
        if len(client.memory) == HORIZON + 1:
    ...
    def _parse_memory(self, init_v, ident, is_over):
        ...
        for idx, k in enumerate(mem):
            ...
            self.queue.put([k.state, k.action, k.prob, k.value, gaelam, 1.0])
            vpred_tp1 = k.value
...
def get_config():
    dirname = os.path.join('train_log', 'train-lezhang-2-{}'.format(ENV_NAME))
